...
```
```{r}
#| label: fig-bubble-ref-sites
#| fig-cap: "Bubble plot of mean abundance at reference sites by year."
#| echo: false
#| warning: false
#| message: false
#| fig-height: 7
...
size_bubble_plot <- ten_min_mean_year %>%
 filter(blockno == block_no) %>%
 ggplot(aes(x = sampyear, y = site, size = mean_ab_n))+
 geom_point(aes(size = mean_ab_n, fill = mean_ab_n), alpha = 0.75, shape = 21) +
 scale_size(range = c(0.5, 20), name = bquote('Average count\n(abalone.10'*~min^-1*')'))+
 theme_bw()+
 scale_color_gradientn(colours = c("blue", "yellow", "red"), aesthetics=c("fill", "color"),
                       name = bquote('Average count\n(abalone.10'*~min^-1*')'))+
 ylab('Site')+
 xlab('Year')+
 facet_grid(. ~ legal_size)

size_bubble_plot

ggsave(filename = paste(ts_plots_folder, paste('/TimedSwimSurvey_', samp_year, '_TenMinuteCount_ReferenceSiteBubblePlot', '.pdf', sep = ''), sep = ''), plot = size_bubble_plot, units = 'mm', width = 190, height = 200)
ggsave(filename = paste(ts_plots_folder, paste('/TimedSwimSurvey_', samp_year, '_TenMinuteCount_ReferenceSiteBubblePlot', '.png', sep = ''), sep = ''), plot = size_bubble_plot, units = 'mm', width = 190, height = 200)
