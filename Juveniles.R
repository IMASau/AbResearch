...
# juv$newdate <- strptime(as.character(juv$survdate), "%d/%m/%Y")
...
ab_count <- juv %>% group_by(location, survdate, string, plate) %>%
summarise(ab_ct =n()) %>%
complete(location, survdate, string, plate, fill = list(ab_ct = 0)) %>%
 as.data.frame()
...
ggplot(Pick, aes(y=Ab_Sum, x=survdate)) +
...
ggplot(BI, aes(y=Ab_Sum, x=survdate, fill=string)) +
