...
load ("D:/R_Stuff/SAM/Logistic/SamResults131016.RData")#load ('D:/R_Stuff/SAM/SAM2016_April.RData') #SAM2016_April
# keep(SamResults, SamListKeep, SamFilter, SamFiltered, sure =T)
#SamFilter<-SamFiltered
...
DT_MSc<-c(171, 172)
pick <- which(SamFilter$SIT_Id == DT_MSc)
DT_MSc<-SamFilter[pick,]
...
SamFilter<-droplevels(subset(SamFilter, SIT_Id != 171))
