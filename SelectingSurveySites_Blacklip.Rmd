...
ts.hex <- filter(sf.ab.hex, zone == 'E' &
                  blockno %in% c(13, 16, 21, 22, 23, 24, 27, 28, 29)|
                  subblockno == '28A') %>%
...
# calculate mean cpue for each oid withing timed swim survey areas and create data frame for later analysis
df_cpue <- ts.hex %>% 
...
write.xlsx(df_cpue, paste(gis_folder_path, paste('/TimedSwimSites_OID_CPUE.xlsx', sep = ""), sep = ''), sheetName = "Sheet1", colNames = TRUE, rowNames = TRUE, append = FALSE)
