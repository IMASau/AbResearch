...
time.swim.dat.df <- time.swim.dat %>% 
 mutate(sizeclass_freq = as.numeric(sizeclass_freq)) %>%
...
# Vessel data for 2025 surveys
morana_gps_2025_a <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-01-15_download.gpx'), layer = 'waypoints')
morana_gps_2025_b <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-01-31_download.gpx'), layer = 'waypoints')
morana_gps_2025_c <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-02-27_download.gpx'), layer = 'waypoints')
morana_gps_2025_d <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-03-14_download.gpx'), layer = 'waypoints')
morana_gps_2025_e <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-06-23_download.gpx'), layer = 'waypoints')
morana_gps_2025_f <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-07-01_download.gpx'), layer = 'waypoints')
morana_gps_2025_g <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-07-07_download.gpx'), layer = 'waypoints')
morana_gps_2025_h <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-07-16_download.gpx'), layer = 'waypoints')
morana_gps_2025_i <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-07-25_download.gpx'), layer = 'waypoints')
morana_gps_2025_j <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-09-22_download.gpx'), layer = 'waypoints')
morana_gps_2025_k <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-09-24_download.gpx'), layer = 'waypoints')
morana_gps_2025_l <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-09-29_download.gpx'), layer = 'waypoints')
morana_gps_2025_m <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-10-07_download.gpx'), layer = 'waypoints')
morana_gps_2025_n <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-10-15_download.gpx'), layer = 'waypoints')
morana_gps_2025_o <- st_read(file.path(gps_downloads_folder, 'MORANAII-2025-10-17_download.gpx'), layer = 'waypoints')

morana_gps_2025 <- bind_rows(morana_gps_2025_a, 
                             morana_gps_2025_b, 
                             morana_gps_2025_c,
                             morana_gps_2025_d,
                             morana_gps_2025_e,
...
morana_gps_2025 <- morana_gps_2025 %>%
 mutate(sampyear = 2025,
        vesselname = 'MoranaII')
##---------------------------------------------------------------------------##
# Combine GPS dataframes

vessel.gps <- bind_rows(morana.gps.2020, morana.gps.ref, morana.gps.2021, taroona.gps.2021,
                        morana.gps.2022, morana_gps_2023, morana_gps_2024, morana_gps_2025) %>%  
...
##---------------------------------------------------------------------------##
# load final proposed sites for 2025
ts_sites_east_final_2025 <- read.xlsx(paste(sprintf("C:/Users/%s/Dropbox (UTAS Research)/DiveFisheries/Abalone/FISdata/FIS_TimedSwimSurveys2025/TimedSwimSites_EastCoast_Final_2025.xlsx", Sys.info()[["user"]]))) %>% .[,-1]

ts_sites_actaeons_final_2025 <- read.xlsx(paste(sprintf("C:/Users/%s/Dropbox (UTAS Research)/DiveFisheries/Abalone/FISdata/FIS_TimedSwimSurveys2025/TimedSwimSites_Block13_Final_2025.xlsx", Sys.info()[["user"]]))) %>% .[,-1]

ts_sites_block21_final_2025 <- read.xlsx(paste(sprintf("C:/Users/%s/Dropbox (UTAS Research)/DiveFisheries/Abalone/FISdata/FIS_TimedSwimSurveys2025/TimedSwimSites_Block21_Final_2025.xlsx", Sys.info()[["user"]]))) %>% .[,-1]

ts_sites_greenlip_final_2025 <- read.xlsx(paste(sprintf("C:/Users/%s/Dropbox (UTAS Research)/DiveFisheries/Abalone/FISdata/FIS_TimedSwimSurveys2025/TimedSwimSites_NEGreenlip_Final_2025.xlsx", Sys.info()[["user"]]))) %>% .[,-1] %>% mutate(oid = as.character(oid))

ts_sites_final_2025 <- bind_rows(ts_sites_east_final_2025, 
                                 ts_sites_actaeons_final_2025, 
                                 ts_sites_block21_final_2025,
                                 ts_sites_greenlip_final_2025)
...
# convert 2025 site data to sf
ts.site.2025.sf <- ts_sites_final_2025 %>% 
 st_as_sf(coords = c("longitude", "latitude"), crs = WGS84) %>%
 # dplyr::rename('site' = 'name') %>% 
 dplyr::select(c(site, geometry, oid)) %>% 
 mutate(sampyear = 2025)
...
##---------------------------------------------------------------------------##
# Combine all years
ts.sites.final.sf <- bind_rows(ts.site.2020.sf, ts.site.2021.sf, ts.site.2022.sf, ts.site.2023.sf, ts.site.2024.sf, ts.site.2025.sf)
##---------------------------------------------------------------------------##
...
# Join sites sampled to renamed site file for 2025
ts.sites.join.2025 <- ts.sites.final.sf %>% 
 filter(sampyear == 2025) %>% 
 left_join(., ts.dat.unique, by = c('site' = 'site',
                                    'sampyear' = 'sampyear'))
##---------------------------------------------------------------------------##
...
ts.dat.site.join <- bind_rows(ts.sites.join.2020, ts.sites.join.2021,
                              ...
                              ts.sites.join.2024, ts.sites.join.2025)
##---------------------------------------------------------------------------##
...
# rejoin straightline distance and direction tables 
...
# join proposed site data to size frequency data
        ...
        ts.dat.prop.2025 <- time.swim.dat %>% 
         filter(sampyear == 2025) %>% 
         left_join(., ts.proposed.geom, by = c('site' = 'site',
                                               'sampyear' = 'sampyear'))
        ...
        ts.dat.prop <- bind_rows(ts.dat.prop.2020, ts.dat.prop.2021, 
                                 ...
                                 ts.dat.prop.2024, ts.dat.prop.2025) %>%   
...
# join actual site data to size frequency data
        ...
        ts.dat.act <- bind_rows(ts.dat.act.2020, ts.dat.act.2021, 
                                ...
                                ts.dat.act.2024, ts.dat.act.2025) %>%    
...
ts.dat.prop.df.2025 <- time.swim.dat.df %>%
 filter(sampyear == 2025) %>%
 left_join(., ts.proposed.geom, by = c('site' = 'site',
                                       'sampyear' = 'sampyear'))
...
ts.dat.prop.df <- bind_rows(ts.dat.prop.df.2020, ts.dat.prop.df.2021, 
                            ...
                            ts.dat.prop.df.2024, ts.dat.prop.df.2025) %>%
        dplyr::select(-c(blockno.y, site.y, site_old)) %>%
        dplyr::rename(proposed.geom = 'geometry',
                      blockno = 'blockno.x') %>%
...
ts.dat.act.df.2025 <- time.swim.dat.df %>%
 filter(sampyear == 2025) %>%
 left_join(., ts.actual.geom, by = c('site' = 'site',
                                     'sampyear' = 'sampyear',
                                     'sampdate'))
...
ts.dat.act.df <- bind_rows(ts.dat.act.df.2020, ts.dat.act.df.2021, 
                           ...
                           ts.dat.act.df.2024, ts.dat.act.df.2025) %>%
        dplyr::select(-c(blockno.y, site_old)) %>%
        dplyr::rename(actual.geom = 'geometry',
                      blockno = 'blockno.x') %>%
...
# Remove duplicate records (#Need to work out where these are being duplicated?)
...
#| message: false
##---------------------------------------------------------------------------##
# 16. Standardise data for extra time #### 
...
time.swim.dat.final <- ts_dat_final_ref %>% 
 mutate(sizeclass_freq = as.numeric(sizeclass_freq),
        sizeclass_freq_10 = round((sizeclass_freq / (time.elapsed) * 10)))
...
urchin_data <- time_swim_meta_dat_final %>% 
 filter(!is.na(starttime) &
...
# Convert 2020 percentage observations to equivalent categorical observation for years 2021-2025
urchin_data_2020 <- urchin_data %>% 
 filter(sampyear == 2020) %>% 
...
urching_data_post <- urchin_data %>% 
 filter(sampyear != 2020)

urchin_data <- bind_rows(urchin_data_2020, urching_data_post)
   
# save dataframe
saveRDS(urchin_data, paste(samp_year_folder, '/urchin_data.RDS', sep = '')) 

st_write(urchin_data,
         dsn = paste(samp_year_folder, '/urchin_data.gpkg', sep = ''),
         layer = "urchin_data", driver = "GPKG", overwrite = T, delete_dsn = T)

##---------------------------------------------------------------------------##

# 14. Convert metadata to gpkg
...
ts_meta_dat <- time_swim_meta_dat_final %>% 
 select(-c(finish_geom)) %>% 
 st_as_sf() %>% 
 dplyr::rename(geometry = 'start_geom')
 # st_transform(WGS84) %>% 
 # extract(geometry, into = c('lattitude', 'longitude'), '\\((.*),(.*)\\)', conv = T)

# Save spatial layer for QGIS
st_write(ts_meta_dat,
         dsn = paste(samp_year_folder, '/ts_meta_dat_', Sys.Date(), '.gpkg', sep = ''),
         layer = "ts_meta_dat", driver = "GPKG", overwrite = T, delete_dsn = T)
